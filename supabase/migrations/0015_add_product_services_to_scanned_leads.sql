...
add column if not exists product_services text;
...
create or replace function public.update_scanned_lead_details(
  ...
  target_product_services text default null
)
returns void
language plpgsql
security definer
...
begin
  ...
  set
    ...
    product_services = nullif(btrim(target_product_services), ''),
    ...
    parse_status = case
      when coalesce(
        ...
        nullif(btrim(target_product_services), ''),
...
revoke all on function public.update_scanned_lead_details(uuid, text, text, text, text, text, text, text) from public;
grant execute on function public.update_scanned_lead_details(uuid, text, text, text, text, text, text, text) to authenticated;
